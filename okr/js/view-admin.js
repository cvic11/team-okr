  ...
  const ViewAdmin = {
    ...
    render() {
      ...
      this.el.innerHTML =
        ...
        + '<button class="lnk" id="adm-cleansim">[데모 활동 청소]</button>'
    ...
    bind() {
      ...
      this.el.querySelectorAll('[data-act="pin"]').forEach(b => b.addEventListener('click', () => {
        ...
        if (pin && /^\d{4}$/.test(pin)) { m.pin = pin; S().persist(); window.R.notice(m.name + ' 핀 변경됨'); }
      ...
      this.el.querySelector('#adm-close-copy').addEventListener('click', () => {
      ...
      this.el.querySelector('#adm-close-empty').addEventListener('click', () => {
      ...
      this.el.querySelector('#adm-cleansim').addEventListener('click', () => {
        const r = S().cleanSim();
        window.R.notice('데모 활동 청소 완료 — 생성 ' + r.removed + '건 제거, 완료 ' + r.reverted + '건 복원 (u 키로 되돌리기 가능)');
      });
      this.el.querySelector('#adm-reset').addEventListener('click', () => {

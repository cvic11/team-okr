...
async function openReflection(entityType,entityId,period,opts){
  ...
  showModal(`
    ...
    <div class="modal-body">
      ...
      <div class="eval-section"><div class="eval-section-label">잘 된 것</div><textarea class="eval-input" data-field="r-worked" rows="3" placeholder="이번 기간 가장 잘 작동한 것 — 결과보다 '왜 잘 됐는지'가 중요">${esc(r.what_worked||'')}</textarea></div>
      <div class="eval-section"><div class="eval-section-label">어려웠던 것</div><textarea class="eval-input" data-field="r-struggled" rows="3" placeholder="목표 미달이라도 솔직하게 — 70% 달성도 OKR에서는 성공">${esc(r.what_struggled||'')}</textarea></div>
      <div class="eval-section"><div class="eval-section-label">배운 것</div><textarea class="eval-input" data-field="r-learned" rows="3" placeholder="이 경험에서 가져갈 인사이트·가설·원리">${esc(r.what_learned||'')}</textarea></div>
      <div class="eval-section"><div class="eval-section-label">다음에 시도할 것</div><textarea class="eval-input" data-field="r-nexttry" rows="2" placeholder="다음 분기에 새로 도전할 1~2가지">${esc(r.next_try||'')}</textarea></div>

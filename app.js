...
const GUIDES={
  objective:{title:'Objective — 분기 도달점의 정의',body:'KPI가 아니라 도달 후 변화의 모습을 적으십시오.',no:['"매출 10% 증가" — 결과 수치는 KR 자리'],yes:['"점주가 본부에 먼저 전화하는 신뢰 관계 구축"'],check:'야심 · 정성 · 시간 제한 · 정서적 동인'},
  kr:{title:'Key Result — 달성 판정의 기준',body:'이 수치가 달성되면 Objective도 100% 성공이라 자신할 수 있는가.',no:['"본부 회의 5회 진행" — 활동(Activity)은 Initiative'],yes:['"점주 만족도 70점 → 80점"'],check:'측정 가능 · 결과 중심 · 70% 달성도 성공'},
  initiative:{title:'Initiative — KR 달성을 위한 액션',body:'KR이 "무엇을"이라면 Initiative는 "어떻게". 완료 후 KR 진척 정도가 그려져야 합니다.',no:null,yes:null,check:'통제 가능 · 1~4주 단위 · KR과 직결'}
};
...
function renderOKR(){
  ...
  return `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px;"><div><h2 style="font-weight:800;font-size:23px;margin:0;">분기 OKR</h2><div style="font-size:13px;color:var(--text-soft);margin-top:2px;">${esc(currentTeam()?.quarter||'')} · O ${state.objectives.length}개 · KR ${totalKR}개</div></div><button class="btn btn-primary" data-act="add-obj">${I.plus} Objective 추가</button></div>${isEmpty?renderGuideCard('objective')+renderGuideCard('kr')+renderGuideCard('initiative')+`<div class="empty"><div style="margin-bottom:6px;">${I.target}</div>분기 도달점을 정의하는 것으로 시작하십시오.</div>`:state.objectives.map((o,i)=>renderObjective(o,i)).join('')}`;
}
